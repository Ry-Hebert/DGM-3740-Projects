<!DOCTYPE html>
<html lang="en">

<?php
$pageTitle = 'Agent 1';
include './_header.php';
?>

<main class="agent-page">
    <section class="agent-sec">
        <h1>Agent-Wendi Wadsworth</h1>
        <div class="agent-body">
            <img class="agent-photo" src="./assets/img/agent1.jpg">
            <div class="agent-item">
                <h3>Phone:</h3>
                <p><a class="nav-link" href="tel:18005551234">1-800-555-1234</a></p>
            </div>
            <div class="agent-item">
                <h3>Working:</h3>
                <p>Monday, Wednesday, Friday</p>
            </div>
            <div class="agent-item">
                <h3>Destination Specialization:</h3>
                <p>Israel, Egypt, Italy, Greece, Hawaii, Guatemala, Southern California, Orlando, Las Vegas, Bahamas, Caribbean cruises.</p>
            </div>
            <div class="agent-item">
                <h3>Office Hours:</h3>
                <p>8:00am-6:00pm</p>
            </div>
            <div class="agent-item">
                <h3>Biography:</h3>
                <p>I have loved travel for as long as I can remember. At the age of 12 I asked if I could fly by myself to Hawaii and I never came back.
                <br><br>
                Right out of high school I started into travel school and have had the opportunity to work in many aspects of the industry. In every aspect I have worked I have had the opportunity to help people travel, to help fulfill people?s dreams, what could be be6er to do every day!</p>
            </div>
            <div class="agent-item">
                <h3>Customer Comment:</h3>
                <p>"Thank you Wendy for all your planning and efforts to ensure that we could enjoy our tour and bring back such wonderful memories."<br>-John Doe<br><br>
                    "Thank you for your cheery a_tude and hard work during the planning of our tour!"<br>-The Smiths<br><br>
                    "Thanks for your pa`ence and efficiency. Let's do it again some`me very soon!"<br>-Jane and John Westagar</p>
            </div>
        </div>
    </section>
</main>

<?php include '_footer.php' ?>
</html>
